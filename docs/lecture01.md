...
モジュールをimportすることで、簡単に新たな機能を追加することが出来ます。
今回は、ランダムな数を生成することが出来る、`random`モジュールを追加してみましょう。

まず、コマンドパレットを開きます。Windowsでは、アプリ検索画面から「cmd」と検索してください。
そして、以下のコマンドを入力して`random`モジュールをインストールします。

```
pip install random
```

これだけで準備は完了です。実際に使ってみましょう。
