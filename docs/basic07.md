---
var:
  header-title: "Pythonで釣りゲームを作ろう 基礎編7　if文"
...
-  [条件分岐とは](basic07.html#条件分岐とは) 
-  [if文](basic07.html#if文) 
...
- **7-1** elifを用いて、所持金が足りなければ`所持金が足りません`というメッセージを表示させる。

- **7-2** 所持金が足りなければ、足りない金額を計算して、`所持金がxx円足りません`というメッセージを表示させる
...
```python{.numberLines caption="7-1答え"}
level = 25 #プレイヤーのレベル
money = 200 #所持金
...
```python{.numberLines caption="7-2答え"}
level = 25 #プレイヤーのレベル
money = 200 #所持金
